...
export default class GitHubConnector extends ConnectorRuntime {
  readonly definition: ConnectorDefinition = {
    ...
    feeds: {
      content: {
        ...
        configSchema: {
          type: 'object',
          required: ['repo_owner', 'repo_name', 'content_type'],
          properties: {
            ...
            labels_filter: {
              type: 'array',
              items: { type: 'string' },
              description: 'Optional label filter for issues/PRs',
            },
            lookback_days: {
              type: 'integer',
              minimum: 1,
    ...
    optionsSchema: {
      ...
      properties: {
        ...
        lookback_days: {
          ...
          maximum: 730,
          default: 365,
          description: 'Initial lookback days for first sync',
        },
      },
    },
  };
  ...
  async sync(ctx: SyncContext): Promise<SyncResult> {
    ...
    const contentType = config.content_type ?? 'issues';
  ...
  private async syncIssuesAndPulls(
    ...
    for (const item of items) {
      ...
      events.push({
        external_id: `${isPR ? 'pr' : 'issue'}_${repo.owner}_${repo.repo}_${item.number}`,
        title: item.title,
  ...
  private async syncIssueComments(
    ...
    const url =
      `https://api.github.com/repos/${repo.owner}/${repo.repo}/issues/comments?${query.toString()}`;
    ...
    return comments
      .map((comment): EventEnvelope | null => {
        ...
        return {
          external_id: `issue_comment_${repo.owner}_${repo.repo}_${comment.id}`,
  ...
  private async syncPullRequestComments(
    ...
    const url =
      `https://api.github.com/repos/${repo.owner}/${repo.repo}/pulls/comments?${query.toString()}`;
    const comments = await this.requestJson<GitHubCommentLike[]>({ url, token });
    ...
    return comments
      .map((comment): EventEnvelope | null => {
        ...
        return {
          external_id: `pr_comment_${repo.owner}_${repo.repo}_${comment.id}`,
          content: comment.body ?? '',
          author: comment.user?.login ?? 'unknown',
          url: comment.html_url,
          published_at: createdAt,
  ...
  private async syncDiscussions(
    ...
    return discussions
      .map((discussion): EventEnvelope | null => {
        ...
        return {
          external_id: `discussion_${repo.owner}_${repo.repo}_${discussion.number}`,
          ...
          published_at: createdAt,
          kind: 'discussion',
          score: toInt(discussion.reactions?.totalCount, 0) + toInt(discussion.comments?.totalCount, 0),
          metadata: {
  ...
  private async syncDiscussionComments(
    ...
    for (const discussion of discussions) {
      ...
      for (const comment of comments) {
        ...
        result.push({
          external_id: `discussion_comment_${repo.owner}_${repo.repo}_${comment.id}`,
